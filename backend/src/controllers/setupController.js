  ...
  try {
    ...
    if (!process.env.SETUP_SECRET) {
      return res.status(500).json({
        success: false,
        message:
          "SETUP_SECRET is not configured on the server. Add it in your hosting dashboard env vars first.",
      });
    }
    ...
    if (!process.env.ADMIN_EMAIL || !process.env.ADMIN_PASSWORD) {
      return res.status(500).json({
        success: false,
        message:
          "ADMIN_EMAIL / ADMIN_PASSWORD is not configured on the server.",
      });
    }
